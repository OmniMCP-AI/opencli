...
| `opencli shopdora product-shopdora-download <shopee-product-url>` | Open `https://www.shopdora.com/my/analysis/comment`, submit the Shopee product link through the add dialog, wait for analysis completion, open the comment-detail page, apply the low-star/media/date filters, download the Excel, and return the local file link |
...
- `product-shopdora-download` waits for Shopdora's task progress API to reach `100`, builds the `newComment` detail URL from `site`, `taskKey`, and `shopId`, then downloads the filtered comment Excel and returns both `task_key` and the local file path/URL.
