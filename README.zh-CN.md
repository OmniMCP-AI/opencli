...
| **shopee** | `product` `product-shopdora-download` | 浏览器 |
...
Shopee template 同步关系：`extractProductDetailFromShopee` 对应 `opencli shopee product`，`exportReviewDataFromShopee` 对应 `opencli shopee product-shopdora-download`。
...
| **Shopee** | 评论导出（CSV） | 从 Shopee 商品页导出 Shopdora 评论数据 |
