...
export interface DaemonCommand {
  id: string;
  action: 'exec' | 'navigate' | 'tabs' | 'cookies' | 'screenshot' | 'close-window' | 'sessions' | 'set-file-input' | 'insert-text' | 'bind' | 'network-capture-start' | 'network-capture-read' | 'cdp' | 'frames' | 'download-wait';
...
export async function bindCurrentTab(
  ...
  const result = await sendCommand('bind', { workspace, ...opts });
