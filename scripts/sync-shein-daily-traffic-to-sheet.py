...
def adapter_row_to_record(row: dict[str, Any], store: str) -> dict[str, Any]:
    record = {
        ...
        "点击率": default_zero_when_blank(ratio(row.get("goods_uv_idx", ""), row.get("eps_uv_idx", ""))),
        ...
        "商品评价数": default_zero_when_blank(row.get("total_comment_cnt", "")),
        "差评数": default_zero_when_blank(row.get("bad_comment_cnt", "")),
        "差评率": default_zero_when_blank(row.get("bad_comment_rate", "")),
        "退货订单数": default_zero_when_blank(row.get("return_order_cnt", "")),
        "退货件数": default_zero_when_blank(row.get("return_qty", "")),
